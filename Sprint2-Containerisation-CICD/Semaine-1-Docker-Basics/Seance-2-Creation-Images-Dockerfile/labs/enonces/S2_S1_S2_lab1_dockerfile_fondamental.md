...
**Méthode 1 - Commandes individuelles (recommandée pour l'apprentissage)** :

```bash
# Premier build (complet) - Mesurer le temps de build initial
time docker build -t flask-lab1:v2 .

# Modification du code seulement (app.py)
# Ajoutez un commentaire dans app.py pour simuler un changement
echo "# Version 2 - $(date)" >> app.py

# Second build (cache utilisé pour requirements) - Observer la différence
time docker build -t flask-lab1:v2 .

# Comparaison des tailles entre versions
docker images | grep flask-lab1
```

**Méthode 2 - Script automatisé (optionnel pour tests répétés)** :

Créez le fichier `test-cache.sh` pour automatiser le test :

```bash
#!/bin/bash
# Script de test d'optimisation du cache Docker
set -e

echo "=== TEST D'OPTIMISATION DU CACHE DOCKER ==="
echo

# Nettoyage initial
echo "1. Nettoyage des images existantes..."
docker rmi -f flask-lab1:v2 2>/dev/null || true

# Premier build complet
echo "2. Premier build (complet)..."
echo "Timestamp: $(date)"
time docker build -t flask-lab1:v2 . 2>&1 | tee build1.log

# Modification du code
echo "3. Modification du code application..."
echo "# Version modifiée - $(date)" >> app.py

# Second build avec cache
echo "4. Second build (avec cache optimisé)..."
echo "Timestamp: $(date)"
time docker build -t flask-lab1:v2 . 2>&1 | tee build2.log
...
docker images | grep flask-lab1
echo
echo "=== ANALYSE DU CACHE ==="
echo "Build 1 (complet) - Toutes les couches créées"
echo "Build 2 (avec cache) - Couches requirements réutilisées"

# Nettoyage des logs
rm -f build1.log build2.log
echo
echo "Test terminé ! Observez la différence de temps de build."
```
